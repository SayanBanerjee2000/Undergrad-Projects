...
if(!empty($_REQUEST['mode']))
{
	...
	$sql = "SELECT * from `user_det` where `loginid` = '$id' and `password`='$old_pass'";
	$res = mysqli_query($con,$sql);
    
	$check=mysqli_num_rows($res);
	if($check == '1')
	{
		$qry1="UPDATE `user_det` set `password`='$new_pass'  where `loginid` ='$id'";
		$rs1=mysqli_query($con,$qry1);
		if($rs1)
		{
			unset($_SESSION['client_login_status']);
			session_destroy();
            $sql_del = "DELETE FROM temp WHERE loginid = '$id'";
